package br.com.jusley.hexagonal.infrastructure.jpa.entities;
...
import br.com.jusley.hexagonal.application.domain.event.EventTicket;
...
public class EventTicketEntity {
    ...
    public static EventTicketEntity of(final EventEntity event, final EventTicket eTicket){
        return new EventTicketEntity(
                UUID.fromString(eTicket.ticketId().value()),
                UUID.fromString(eTicket.customerId().value()),
                eTicket.ordering(),
                event
        );
    }
    ...
    public EventEntity event() { return event;}
